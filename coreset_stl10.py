...
def extract_embeddings(model, indices, query_dataset):
    """Extract normalized MobileNet backbone embeddings for core-set selection."""
    model.eval()
...
# ==========================================
# 5. Core-Set Query Strategy
# ==========================================
def query_coreset(model, labeled_indices, unlabeled_indices, query_dataset, query_size):
    """
    k-Center Greedy in feature space.
    Select unlabeled points that maximize coverage of the current labeled set.
    """
    if not unlabeled_indices:
        return []

    num_to_select = min(query_size, len(unlabeled_indices))

    labeled_features = extract_embeddings(model, labeled_indices, query_dataset)
    unlabeled_features = extract_embeddings(model, unlabeled_indices, query_dataset)

    if len(labeled_indices) > 0:
        min_distances = torch.cdist(unlabeled_features, labeled_features).min(dim=1).values
    else:
        min_distances = torch.full((len(unlabeled_indices),), float("inf"))

    selected_local = []
    for _ in range(num_to_select):
        next_local_idx = torch.argmax(min_distances).item()
        selected_local.append(next_local_idx)

        new_center = unlabeled_features[next_local_idx : next_local_idx + 1]
        distances_to_new_center = torch.cdist(unlabeled_features, new_center).squeeze(1)
        min_distances = torch.minimum(min_distances, distances_to_new_center)
        min_distances[next_local_idx] = -1.0
...
def run_experiment(
    ...
    wandb.init(
        ...
        config={
            ...
            "query_strategy": "k-center-greedy",
            "feature_space": "mobilenet_v3_small_backbone",
        },
        reinit=True,
    )
    ...
    for iteration in range(1, AL_ITERATIONS + 1):
        ...
        queried = query_coreset(model, labeled_indices, unlabeled_list, query_dataset, QUERY_SIZE)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Core-set Active Learning on STL-10")
    parser.add_argument(
        "--seeds",
        type=int,
        nargs="+",
    ...
    for seed in args.seeds:
        run_name = f"run_coreset_seed{seed}"
        print(f"\n{'=' * 55}\n  {run_name}\n{'=' * 55}")
        accs = run_experiment(
            seed=seed,
    ...
    print(f"\n{'=' * 55}")
    print(f" SUMMARY - CORESET_KCENTER ({len(args.seeds)} seeds)")
    print(f"{'=' * 55}")
    ...
    wandb.init(
        project="stl10-active-learning",
        name="summary_coreset_kcenter",
        config={"method": "coreset_kcenter", "num_seeds": len(args.seeds)},
        reinit=True,
    )
    for i, (m, s) in enumerate(zip(mean_accs, std_accs)):
        wandb.log(
